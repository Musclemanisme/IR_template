...
def get_scheduler(opt, optimizer):
    if opt.scheduler == 'cos':
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt.epochs, eta_min=opt.lr * 0.01)
    elif opt.scheduler == 'step':
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=opt.epochs // 3, gamma=0.5)
    elif opt.scheduler == 'exp':
        scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.99)
    elif opt.scheduler =='mul':
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer,[100, 200, 300, 400], gamma=0.66)
